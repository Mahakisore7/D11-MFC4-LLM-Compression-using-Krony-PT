...
class CompressionAgent:
    ...
    def analyze_and_compress_layer(self, layer_idx, layer_name, W):
        ...
        Residual = W - W_approx
        sparse_thresh = torch.quantile(torch.abs(Residual), 0.95) # Keep top 5% outliers
    ...
    def run(self):
        ...
        # Loop through all 12 layers of GPT-2
        for i in range(len(self.model.h)):
            # Target the MLP (FFN) Projection Layer
            # This is the huge 2.3M param matrix
        ...
        # Get file sizes to show off
        orig_size = os.path.getsize('compressed_gpt2.pt') # Approximation
...
if __name__ == "__main__":
    agent = CompressionAgent(energy_threshold=0.70)
    agent.run()